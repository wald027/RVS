...
TRAINED_MODEL_PATH = BASE_DIR + "modelo_teste_final"  #"model_6D_3_SEMHISTORICO_TODOS_OS_DADOS" #modelo_teste_final
#now = datetime.now()
#formatted_time = now.strftime("%d%m%Y_%H%M%S")
#TRAINED_MODEL_PATH = BASE_DIR + f"Model_{str(formatted_time).split()[0].replace('-', '')}"
# Nome do servidor SQL
server = 'PT-L163255\SQLEXPRESS01'
#server = queryByNameDict('',dictConfig)
...
#df_original.to_excel(r"IntelligentProcessAutomationNLP\ModelNLP\training\dataframe.xlsx")
#df_original = pd.read_excel(r"IntelligentProcessAutomationNLP\ModelNLP\training\dataframe.xlsx")
#file_path = r"IntelligentProcessAutomationNLP\ModelNLP\training\dataframe.xlsx"

df_original = pd.read_excel(r"C:\Users\nayara.rodrigues\Documents\RVS-main\IntelligentProcessAutomationNLP\ModelNLP\training\Resultados_17092024_100121.xlsx")
print(df_original)

#if os.path.exists(file_path):
 #   os.remove(file_path)
#else:
 #   raise Exception("Nenhum excel ficheiro foi encontrado.")
...
df_original['Nome Intenção'] = '' # label da predição do modelo             #df_original["Label Intenção"].map(label_map) 
df_original['Score'] = ''
...
X = df_filtered["Body"].fillna("") #df_filtered["Subject"].fillna("") + " - " + df_filtered["Body"].fillna("")
...
)#.to(DEVICE) # add it when you use GPU
#print(train_)
...
clf = pipeline("text-classification", model=model, tokenizer=tokenizer, truncation=True, padding="max_length", max_length=128)#, device=DEVICE) # add it when you use GPU
